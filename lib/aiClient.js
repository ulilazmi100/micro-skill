  ...
  micro_lessons: [
    {
      ...
      full_guide: `Objective — Reproduce the failing UI behavior quickly.

Steps:
1) Open the app and locate the failing button.
2) Note browser console errors and network activity.
3) Reduce to one page/component that reproduces the issue.
4) Save reproduction steps and a screenshot.
...
Time estimate: 10–20 minutes.`
    },
    {
      ...
      full_guide: `Objective — Create a failing test that demonstrates the bug.

Steps:
1) Set up test runner environment for the component.
2) Mock required services and inputs.
3) Write an assertion that fails due to the bug.
4) Run the test and commit the failing test.
...
Time estimate: 15–30 minutes.`
    },
    {
      ...
      full_guide: `Objective — Implement a focused fix for the failing test.

Steps:
1) Locate the smallest module responsible.
2) Make one small change and run tests.
3) Ensure no other tests fail.
4) Add brief comments and a changelog line.
...
Time estimate: 10–30 minutes.`
    },
    {
      ...
      full_guide: `Objective — Confirm changes do not break the wider codebase.

Steps:
1) Run the full test suite locally.
2) Fix incidental failures or flakiness.
3) Run linter and fix style issues.
4) Re-run tests and push.
...
Time estimate: 15–60 minutes.`
    },
    {
      ...
      hints: ['Use a short checklist in the PR description: Reproduce, Test, Fix, Verify.'],
      practice_task: 'Prepare PR description and attach a short demo screenshot or GIF.',
      example_output: 'PR ready with demo',
      full_guide: `Objective — Create a clear PR that reviewers can verify quickly.

Steps:
1) Add PR title and concise description.
2) Include reproduction steps and test results.
3) Attach a short demo GIF or screenshots.
4) Add reviewers and request quick feedback.
