...
// Load demo fixture from prompts/demo_fixture.js (single source for demo)
function loadDemoFixtureSync() {
  const demoPath = path.join(process.cwd(), 'prompts', 'demo_fixture.js');
  // eslint-disable-next-line global-require, import/no-dynamic-require
  const mod = require(demoPath);
  return mod;
}

const DEMO_FIXTURE = loadDemoFixtureSync(); // single source for demo
...
// Provider implementations (callOpenAI, callHuggingFace, callGemini) remain the same as before.
// For brevity, I'll include them unchanged (you can copy your existing implementations).
// ... (The same callOpenAI, callHuggingFace, callGemini functions you already had) ...

// --- BEGIN provider functions (unchanged) ---
...
// Public API: generateForProvider (JSON output)
